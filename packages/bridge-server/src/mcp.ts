...
/** 画布地址：daemon 的实际监听地址（多项目并存时端口可能不是 4399） */
function address(): string {
  return daemon.canvasAddress();
}
...
/** 推送节点到画布（经 daemon） */
async function pushNode(partial: Omit<CanvasNode, 'id' | 'createdAt'>): Promise<CanvasNode> {
  return daemon.addNode(partial);
}

/** 推断素材 MIME，daemon 侧会再次校正 */
...
export function createMcpServer(): McpServer {
  ...
  server.tool('canvas_open', '打开/获取画布的浏览器地址。用户想查看画布时调用。', {}, async () => {
    ...
    return {
      content: [{ type: 'text', text: `画布地址：${address()}\n请在浏览器中打开以查看内容。` }],
    };
  });
  ...
  server.registerTool(
    'canvas_show',
    {
      ...
      _meta: {
        ...
        ui: {
          ...
          csp: {
            // 允许内嵌 iframe 从本地 daemon 加载画布 JS/CSS 与本地资产
            resourceDomains: ['http://127.0.0.1:*', 'http://localhost:*'],
          },
        },
      },
    },
    async () => {
      await daemon.ensureDaemon();
      return {
        content: [{ type: 'text', text: `画布已打开。若客户端不支持内嵌，请访问：${address()}` }],
  ...
  server.tool(
    ...
    async ({ url, title }) => {
      const node = await pushNode({ kind: 'image', title, sourcePath: url, mime: guessMime(url) });
  ...
  server.tool(
    ...
    async ({ kind, url, title }) => {
      const node = await pushNode({ kind, title, sourcePath: url, mime: guessMime(url) });
  ...
  server.tool('canvas_list', '列出画布上当前所有组件的摘要。', {}, async () => ({
    content: [{ type: 'text', text: formatNodes(await daemon.listNodes()) }],
  }));
  ...
  server.tool(
    ...
    async ({ id, path: p }) => {
      const result = await daemon.relink(id, p);
      if ('error' in result) {
        return { content: [{ type: 'text', text: `修复失败：${result.error}` }] };
      }
      const text = result.missing ? `已更新路径，但新路径仍不存在：${p}` : `已修复素材指向：${p}`;
  ...
  server.tool(
    ...
    async ({ nodes }) => {
      await daemon.enqueue(nodes as CanvasNode[]);
      return {
        content: [
  ...
  server.tool(
    ...
    {},
    async () => {
      const nodes = await daemon.drainQueue();
      const text =
        nodes.length === 0
  ...
  server.prompt('canvas-open', '打开对话画布', async () => {
    ...
    return {
      messages: [
        {
          role: 'user',
          content: { type: 'text', text: `请打开画布查看：${address()}` },
        },
      ],
    };
  });
  ...
  server.resource(
    ...
    async (uri) => ({
      contents: [{ uri: uri.href, text: formatNodes(await daemon.peekQueue()) }],
    })
  );
  ...
  server.resource(
    ...
    async (uri) => ({
      contents: [{ uri: uri.href, text: formatNodes(await daemon.listNodes()) }],
    })
  );
        ...
        contents: [
          {
            ...
            _meta: {
              ui: {
                csp: { resourceDomains: ['http://127.0.0.1:*', 'http://localhost:*'] },
                preferredSize: { width: 1280, height: 800 },
              },
            },
          },
...
export async function startMcp(): Promise<void> {
  ...
  // 提前拉起 daemon，避免首次工具调用等待启动
  try {
    const state = await daemon.ensureDaemon();
    log(`daemon ready at http://${state.host}:${state.port}`);
  } catch (e) {
    // 不阻断 MCP 启动：后续工具调用会重试并给出可读错误
    log('daemon 预启动失败（后续调用会重试）:', (e as Error).message);
  }
}
